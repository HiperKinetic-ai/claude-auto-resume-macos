...
note "  3) When a session hits the 5-hour limit, run this IN A TERMINAL (not the Claude Code"
note "     prompt) from that project dir:   auto-resume"
note "     (inside Claude Code you can instead run:  ! ~/.claude/resume-at-reset.sh )"
